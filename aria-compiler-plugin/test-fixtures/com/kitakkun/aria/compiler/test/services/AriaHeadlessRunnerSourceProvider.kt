...
/**
 * Opt-in directive: `// WITH_ARIA_HEADLESS` at the top of a box testData file
 * adds the helper source below to that module's compilation. Tests then drive
 * a presenter with:
 *
 *     AriaHeadlessRunner().use { runner ->
 ...
 */
object AriaDirectives : SimpleDirectivesContainer() {
    val WITH_ARIA_HEADLESS by directive("Include the headless Compose runner helper")
}

class AriaHeadlessRunnerSourceProvider(testServices: TestServices) : AdditionalSourceProvider(testServices) {
    override val directiveContainers: List<DirectivesContainer> = listOf(AriaDirectives)
    ...
    override fun produceAdditionalFiles(
        ...
        if (!containsDirective(globalDirectives, module, AriaDirectives.WITH_ARIA_HEADLESS)) {
            return emptyList()
        }

        val dir = testServices.temporaryDirectoryManager.getOrCreateTempDirectory("ariaHeadless")
    ...
    private companion object {
        const val HELPER_FILE_NAME = "AriaHeadlessRunner.kt"
        ...
        val HELPER_SOURCE: String = """
            ...
            fun <E, S, Eff> runHeadless(
                present: @Composable (Flow<E>) -> Aria<S, Eff>,
