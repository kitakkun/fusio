...
class FusioMapToChecker(compat: CompatContext) :
    ...
    override fun check(declaration: FirClass) {
        ...
        val mapToAnnotation = declaration.getAnnotationByClassId(FusioClassIds.MAP_TO, context.session) ?: return

        val targetType = mapToAnnotation.kclassArg(Name.identifier("target"), context.session)
            ?: run {
                reporter.reportOn(declaration.source, FusioErrors.MAP_TO_INVALID_TARGET, "Missing target argument", context)
                return
            }
        val targetClassId = targetType.classId ?: run {
            reporter.reportOn(declaration.source, FusioErrors.MAP_TO_INVALID_TARGET, "Target must be a class reference", context)
            return
        }

        val targetClass = resolveClassById(targetClassId, context.session)
        if (targetClass == null) {
            reporter.reportOn(declaration.source, FusioErrors.MAP_TO_INVALID_TARGET, "Cannot resolve target class: $targetClassId", context)
            return
        }
