...
class FusioMapFromChecker(compat: CompatContext) :
    ...
    override fun check(declaration: FirClass) {
        ...
        val mapFromAnnotation = declaration.getAnnotationByClassId(FusioClassIds.MAP_FROM, context.session) ?: return

        val sourceType = mapFromAnnotation.kclassArg(Name.identifier("source"), context.session)
            ?: run {
                reporter.reportOn(declaration.source, FusioErrors.MAP_FROM_INVALID_SOURCE, "Missing source argument", context)
                return
            }
        val sourceClassId = sourceType.classId ?: run {
            reporter.reportOn(declaration.source, FusioErrors.MAP_FROM_INVALID_SOURCE, "Source must be a class reference", context)
            return
        }

        val sourceClass = resolveClassById(sourceClassId, context.session)
        if (sourceClass == null) {
            reporter.reportOn(declaration.source, FusioErrors.MAP_FROM_INVALID_SOURCE, "Cannot resolve source class: $sourceClassId", context)
            return
        }
