 ...
 * `Presentation` read `.state` and `.effectFlow` directly and don't need
 * destructuring or `copy()`. The explicit [equals] / [hashCode] / [toString]
 * below retain the ergonomics that actually matter (structural equality in
 * tests and readable log output) without the ABI tax.
 *
 * ## Why `@Stable`
 *
 ...
 * both properties are `val`s set once at construction, the instance never
...
public class Presentation<State, Effect>(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return state == other.state && effectFlow == other.effectFlow
    }
    ...
    override fun toString(): String = "Presentation(state=$state, effectFlow=$effectFlow)"
}
