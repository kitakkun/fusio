...
/**
 * Guards the no-plugin fallback of `mappedScope`: if the Fusio compiler plugin
...
class MappedScopeStubTest {
    ...
    fun stub_carries_the_plugin_missing_error_message_in_its_classfile() {
        val bytes = javaClass.classLoader
            .getResourceAsStream("com/kitakkun/fusio/MappedScopeKt.class")!!
            .use { it.readBytes() }
        val text = String(bytes, Charsets.ISO_8859_1)

        assertTrue(
            "mappedScope requires the Fusio Compiler Plugin" in text,
