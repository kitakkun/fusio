...
- Event-handler exhaustiveness: an FIR checker fires on each `buildPresenter<E, F, S> { … }` call. Every sealed subtype of `E` must be reached by some `on<T>` (with `T :> subtype`) or routed via `@MapTo` to a fused child. Severity from `fusio { eventHandlerExhaustiveSeverity = … }` DSL (or `-Pfusio.event-handler-exhaustive-severity=error|warning|none`); default WARNING. Sub-presenter declaration sites aren't currently checked — `FirSimpleFunction` was renamed mid-2.3.x and a per-version compat shim isn't pulling its weight yet.
