...
/**
 * Guards the no-plugin fallback of `mappedScope`: if the Aria compiler plugin
 * isn't registered, every mappedScope call site must throw with a helpful
 ...
 *
 * We used to also reflectively invoke the stub, but after making mappedScope
 ...
 */
class MappedScopeStubTest {

    @Test
    fun stub_carries_the_plugin_missing_error_message_in_its_classfile() {
        val bytes = javaClass.classLoader
            .getResourceAsStream("com/kitakkun/aria/MappedScopeKt.class")!!
            .use { it.readBytes() }
        val text = String(bytes, Charsets.ISO_8859_1)

        assertTrue(
            "mappedScope requires the Aria Compiler Plugin" in text,
            "Error message missing from MappedScopeKt bytecode. Did the stub body change?",
        )
        assertTrue(
            "com.kitakkun.aria" in text,
