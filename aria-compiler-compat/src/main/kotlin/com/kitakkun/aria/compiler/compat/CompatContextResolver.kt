...
object CompatContextResolver {
     ...
     * release Aria hasn't been cut against yet.
     */
    ...
    private fun buildNoMatchingFactoryMessage(
        ...
        append("Aria: no CompatContext implementation covers Kotlin $version (compiler reports ")
        append(KotlinCompilerVersion.VERSION)
        append("). ")
        if (factories.isEmpty()) {
            append(
                "No CompatContext.Factory was discovered on the classpath at all — this usually " +
                    "means the aria-compiler-plugin jar was repackaged without merging " +
            ...
            append(". ")
            append(
                "Either downgrade your Kotlin version, or upgrade Aria to a release that " +
