...
The library's core data type, `Presentation<State, Event, Effect>`, *is* that fusion: a state value, the effect stream the presenter produced alongside it, and a `send: (Event) -> Unit` entry point the UI uses to push input back in. The order is the consumer's reading order — `state` is what the UI renders every frame, `event` is what it sends back on every interaction, `effect` is what it collects once via `LaunchedEffect`.
...
- `buildPresenter { … }` — screen-level entry, returns `Presentation<State, Event, Effect>` (UI calls `presentation.send(event)` to drive input)
...
Because sub-presenters return plain `State` (not `Presentation<State, Event, Effect>`),
